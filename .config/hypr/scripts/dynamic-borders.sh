...
function handle {
    ...
    then
        ...
        do
            ...
            hyprctl dispatch setprop address:$(echo $window | xargs) noborder 0
        done
    elif [[ ${1:0:18} == "closelayer>>waybar" ]]
    then
        # Handle all the other workspaces with only one window
        single_window_workspaces=$(hyprctl workspaces -j | jq '.[] | select(.windows == 1)' | jq ".id")
        for workspace in $single_window_workspaces
    ...
    then
        ...
        then
            ...
            if [[ $floating_status == "false" && ! "$(pgrep -x "waybar")" ]]
            then
    ...
    then
        ...
        then
            ...
            if [[ $no_windows -eq 1 && ! "$(pgrep -x "waybar")" ]]
            then
